...
#define SERIAL  1   // set to 1 to also report readings on the serial port ORIG 0
#define DEBUG   1   // set to 1 to display each loop() run and PIR trigger ORIG 0

#define SHT11_PORT1  1   // defined if SHT11 is connected to a port
#define SHT11_PORT2  2   // defined if SHT11 is connected to a port
#define SHT11_PORT3  3   // defined if SHT11 is connected to a port
#define SHT11_PORT4  4   // defined if SHT11 is connected to a port

#define HYT131_PORT 0   // defined if HYT131 is connected to a port
...
struct {
    byte humi1;      // humidity: 0..100
    int temp1;       // temperature: -500..+500 (tenths)
    byte humi2;      // humidity: 0..100
    int temp2;       // temperature: -500..+500 (tenths)    
    byte humi3;      // humidity: 0..100
    int temp3;       // temperature: -500..+500 (tenths)
    byte humi4;      // humidity: 0..100
    int temp4;       // temperature: -500..+500 (tenths)        
    byte light;     // light sensor: 0..255
    byte count = 0;  // counter: 0..255        
} payload;
...
SHT11 sht11_1 (SHT11_PORT1);
SHT11 sht11_2 (SHT11_PORT2);
SHT11 sht11_3 (SHT11_PORT3);
SHT11 sht11_4 (SHT11_PORT4);
...
static void doMeasure() {
    //byte payload.humi1 = payload.humi2 = payload.humi3 = payload.humi4 = 0; // special case to init running avg
    
    //payload.lobat = rf12_lowbat();
    float h, t;
 
    // measure humi1, temp1
    sht11_1.measure(SHT11::HUMI, shtDelay);        
    sht11_1.measure(SHT11::TEMP, shtDelay);     
    sht11_1.calculate(h, t);
    int humi = h + 0.5, temp = 10 * t + 0.5;
    payload.humi1 = smoothedAverage(payload.humi1, humi, firstTime);
    payload.temp1 = smoothedAverage(payload.temp1, temp, firstTime);

    // measure humi2, temp2
    sht11_2.measure(SHT11::HUMI, shtDelay);        
    sht11_2.measure(SHT11::TEMP, shtDelay);
    sht11_2.calculate(h, t);
    humi = h + 0.5, temp = 10 * t + 0.5;
    payload.humi2 = smoothedAverage(payload.humi2, humi, firstTime);
    payload.temp2 = smoothedAverage(payload.temp2, temp, firstTime);

    // measure humi3, temp3
    sht11_3.measure(SHT11::HUMI, shtDelay);        
    sht11_3.measure(SHT11::TEMP, shtDelay);
    sht11_3.calculate(h, t);
    humi = h + 0.5, temp = 10 * t + 0.5;
    payload.humi3 = smoothedAverage(payload.humi3, humi, firstTime);
    payload.temp3 = smoothedAverage(payload.temp3, temp, firstTime);

    // measure humi4, temp4
    sht11_4.measure(SHT11::HUMI, shtDelay);        
    sht11_4.measure(SHT11::TEMP, shtDelay);
    sht11_4.calculate(h, t);
    humi = h + 0.5, temp = 10 * t + 0.5;
    payload.humi4 = smoothedAverage(payload.humi4, humi, firstTime);
    payload.temp4 = smoothedAverage(payload.temp4, temp, firstTime);       
...
static void doReport() {
    ...
    rf12_sleep(RF12_SLEEP);
        ...
        Serial.print("ROOM ");
        Serial.print(' ');
        Serial.print((int) payload.humi1);
        Serial.print(' ');
        Serial.print((int) payload.temp1);
        Serial.print(' ');
        Serial.print((int) payload.humi2);
        Serial.print(' ');
        Serial.print((int) payload.temp2);
        Serial.print(' ');
        Serial.print((int) payload.humi3);
        Serial.print(' ');
        Serial.print((int) payload.temp3);
        Serial.print(' ');        
        Serial.print((int) payload.humi4);
        Serial.print(' ');
        Serial.print((int) payload.temp4);
        Serial.print(' ');        
        Serial.print((int) payload.light);        
        Serial.print(' ');        
        Serial.print((int) payload.count);          
